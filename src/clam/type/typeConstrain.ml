...
and constrain_union_1 pos sub sup =
  list_iter (Utils.flip (constrain_union_2 pos) sup) sub.union

and constrain_union_2 pos sub sup =
  list_iter (constrain_inter_1 pos sub) sup.union

and constrain_inter_1 pos sub sup =
  list_iter (constrain_inter_2 pos sub) sup.inter

and constrain_inter_2 pos sub sup =
  list_iter (Utils.flip (constrain_base pos) sup) sub.inter
...
and constrain_base pos sub sup =
  ...
  | Record sub_record, Record sup_record ->
    iter_map (fun sup_attr -> constrain_record_attr pos sub_record sup_attr) sup_record.attrs
  | AbsExpr sub_abs, AbsExpr sup_abs ->
    let* () = constrain pos sup_abs.param sub_abs.param in
    let* () = constrain pos sub_abs.ret sup_abs.ret in
    return ()
  | AbsTypeExpr sub_abs, _ ->
    let* _ = with_var (fun var ->
      let* () = constrain pos var sub_abs.param.bound in
      let* ctx = get_context in
      let ret = TypeSystem.substitute_arg ctx sub_abs.param.bind var sub_abs.ret in
      let* () = constrain pos ret (Type.base sup) in
      return ret
    ) in
    return ()
  | _, AbsTypeExpr sup_abs ->
    let* _ = with_var (fun var ->
      let* () = constrain pos var sup_abs.param.bound in
      let* ctx = get_context in
      let ret = TypeSystem.substitute_arg ctx sup_abs.param.bind var sup_abs.ret in
      let* () = constrain pos (Type.base sub) ret in
      return ret
    ) in
    return ()
  | _, Top | Bot, _ | Unit, Unit | Bool, Bool | Int, Int | String, String ->
    return ()
  | _, _ ->
    TypeError.infer_constrain pos (Type.base sub) (Type.base sup)
